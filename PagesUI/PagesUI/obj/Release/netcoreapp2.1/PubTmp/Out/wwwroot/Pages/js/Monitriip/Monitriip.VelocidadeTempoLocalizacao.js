var MonitriipVelocidadeTempoLocalizacao = new function () {
    this.GetByDate = function (pUrl, pModel) {
        $.ajax({
            type: "POST",
            url: pUrl,
            data: {
                pVelocidadeTempoLocalizacaoMirrorModel: pModel
            },
            ...
            success: function (data) {
                if (data.Success) {
                    if (data.Model.length > 0) {
                        MonitriipVelocidadeTempoLocalizacao.InitializeTableWithExportOptions(data.Model);
                        display('table-velocidadetempolocalização', true);
                        display('contentErrorId', false);
                        $("#grid-search").removeAttr("style");
                    }
                    else {
                        display('table-velocidadetempolocalização', false);
    ...
    this.InitializeTableWithExportOptions = function (model) {
        ...
        var settings = {
            ...
            "columns": [
                ...
                {
                    "width": "15%", "data": function (data) {
                        return Core.FormatRound(data["VelocidadeAtual"]) + " km/h";
                    }
                },
                { "width": "10%", "data": "Imei" },
                {
