...
var Transfer = (function ($) {
    ...
    function transfer(settings) {
        ...
        // 总个数显示文本
        var total_num_str = settings.data.length + " Linhas";

        // 分组总个数
        var total_group_num = getGroupNum(groupData, groupListName);
        // 分组总个数显示文本
        var total_group_num_str = total_group_num + " Linhas";
        ...
        var selectAllLeft = "#chkSelectAllLeft";

        var selectAllRight = "#chkSelectAllRight";
        ...
        $(transferId).find(tabItemName).on("click", function () {
            ...
            if (!$(this).is(".tab-active")) {
                ...
                // 清空右侧数量
                $(transferId).find(selectedTotalNum).text("0 Linhas");
                // 未选中项
                if ($(transferId).find(tabContentFirst).css("display") != "none") {
                    ...
                    $(transferId).find(groupTotalNum).empty();
                    $(transferId).find(groupTotalNum).append($(transferId).find(transferDoubleGroupListLiUlLi).length + " Linhas");
                } else {
                    // 分组
                    ...
                    $(transferId).find(totalNum).empty();
                    $(transferId).find(totalNum).append($(transferId).find(transferDoubleListLi).length + " Linhas");
                }
                // 数据变化触发回调
        ...
        $(addSelected).on("click", function () {
            ...
            if ($(selectAllLeft).prop('checked'))
                $(selectAllLeft).prop('checked', false).change();
            ...
            if ($(transferId).find(tabContentFirst).css("display") != "none") {
                ...
                var new_total_num_str = new_group_total_num + " Linhas";
                // 左侧数量
                $(transferId).find(groupTotalNum).append(new_total_num_str);
                // 右侧数量
                $(transferId).find(selectedTotalNum).text(selected_total_num + " Linhas");
                ...
                // 计算右侧总数
                selected_total_num = selectedItemNum;
                var new_total_num_str = new_total_num + " Linhas";
                // 左侧数量
            ...
            $('[id^="selectedCheckbox_"]').click(function () {
                var listUnchecked = $('[class*=checkbox-selected-item-]').not(':checked');

                if (listUnchecked.length > 0 && $(selectAllRight).prop('checked'))
                    $(selectAllRight).prop('checked', false).change();

                if (listUnchecked.length === 0 && !$(selectAllRight).prop('checked'))
                    $(selectAllRight).prop('checked', true).change();
            });
        });
        ...
        $(deleteSelected).on("click", function () {
            ...
            if ($(selectAllRight).prop('checked'))
                $(selectAllRight).prop('checked', false).change();
        ...
        $(selectAllLeft).on("click", function () {
            if ($(this).is(':checked')) {
                $('[class*=checkbox-item-]').prop('checked', true);
                $(addSelected).addClass("btn-arrow-active");
            }
            else {
                $('[class*=checkbox-item-]').prop('checked', false);
                $(addSelected).removeClass("btn-arrow-active");
            }
        });
        ...
        $(selectAllRight).on("click", function () {
            if ($(this).is(':checked')) {
                $('[class*=checkbox-selected-item-]').prop('checked', true);
                $(deleteSelected).addClass("btn-arrow-active");
            }
            else {
                $('[class*=checkbox-selected-item-]').prop('checked', false);
                $(deleteSelected).removeClass("btn-arrow-active");
            }
        });

        $('[id^="itemCheckbox_"]').click(function () {
            var listUnchecked = $('[class*=checkbox-item-]').not(':checked');

            if (listUnchecked.length > 0 && $(selectAllLeft).prop('checked'))
                $(selectAllLeft).prop('checked', false).change();

            if (listUnchecked.length === 0 && !$(selectAllLeft).prop('checked'))
                $(selectAllLeft).prop('checked', true).change();
        });
    }
    ...
    function generateTransfer(inputId, currentTimeStr) {
        var htmlStr =
            ...
            + '<div class="param-item-left">Linhas</div>'
            + '</div>'
            ...
            + '<input class="transfer-double-list-search-input" type="text" id="groupListSearch_' + currentTimeStr + '" placeholder="Pesquisar2" value="" />'
            + '</div>'
            + '</div>'
            ...
            + '<input id="chkSelectAllLeft" type="checkbox" class="custom-control-input checkbox-transfer"><span style="box-shadow: none;" class="custom-control-indicator"></span>'
            + '</label>'
            + '</div>'
            + '<div class="transfer-double-list-search col-md-11">'
            + '<input class="transfer-double-list-search-input" type="text" id="listSearch_' + currentTimeStr + '" placeholder="Pesquisar" value="" />'
            + '</div>'
            + '</div>'
            + '</div>'
            + '<div class="transfer-double-list-content">'
            ...
            + '<div class="transfer-double-list-footer">'
            + '<div>'
            + '<label for="selectAll_' + currentTimeStr + '" class="total_num_' + currentTimeStr + '"></label>'
            + '</div>'
            + '</div>'
            + '</div>'
            + '</div>'
            ...
            + '<div class="param-item">Linhas do Grupo</div>'
            + '</div>'
            ...
            + '<div class="col-md-1">'
            + '<label class="custom-control custom-checkbox">'
            + '<input id="chkSelectAllRight" type="checkbox" class="custom-control-input checkbox-transfer"><span style="box-shadow: none;" class="custom-control-indicator"></span>'
            + '</label>'
            + '</div>'
            + '<div class="transfer-double-list-search col-md-11">'
            + '<input class="transfer-double-selected-list-search-input" type="text" id="selectedListSearch_' + currentTimeStr + '" placeholder="Pesquisar" value="" />'
            + '</div>'
            + '</div>'
            + '</div>'
