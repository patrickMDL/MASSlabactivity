...
function formataHora(campo, evt) {
    var attrReadonly = campo.attributes.readonly;
    //HH:mm   

    //Valida quando o campo é readonly
    if (attrReadonly !== undefined && attrReadonly.name === "readonly") {
        event.returnValue = false;
    }
    else if (event.keyCode < 48 || event.keyCode > 57) {//Valida se foi digitado letras
        event.returnValue = false;
    }
    else {
        xPos = PosicaoCursor(campo);
        ...
        if (tam > 2 && tam < 5) {
            ...
            var hh = arVr[0] + arVr[1];
            var mm = arVr[2] + arVr[3];

            //Validar Hora e minuto corretos
        ...
        if (tam === 4) {
            ...
            if (xPos === 2) {
                campo.selectionStart = pos + 1;
                campo.selectionEnd = pos + 1;
            }
            else {
                campo.selectionStart = pos;
                campo.selectionEnd = pos;
            }
        }
    }
}
...
// Auto Digita Pontos 3 Casas Decimais
function format3casasDecimais(fld, milSep, decSep, e) {
    var sep = 0;
    var key = '';
    var i = j = 0;
    var len = len2 = 0;
...
// Formata o campo para aceitar somente números
function olyNumber(num) {
