function AdministratorDevices() {
    this.GetDevices = function (pUrl) {
        $.ajax({
            type: "GET",
            url: pUrl,
            ...
            success: function (data) {
                if (data.Success) {
                    AdministratorDevices.InitializeTableWithExportOptions(data.Model);
                    $("#grid-search").removeAttr("style");
                }
            }
        });
    },
	this.GetDeviceByUId = function (pUrl, pDeviceUId) {
	    $.ajax({
	        type: "GET",
	        url: pUrl,
	        data: {
	            pDeviceUId: pDeviceUId
	        },
	        content: "application/json; charset=utf-8",
	        dataType: "json",
	        success: function (data) {
    ...
    this.InitializeTableWithExportOptions = function (model) {
        ...
        var settings = {
            ...
            "columns": [
                { "width": "40%", "data": "Name" },
                { "width": "20%", "data": "ProtocolTypeName" },
                {
                    "width": "15%", "data": function (data) {
                        if (data["ProtocolType"] === 1) {
                            return Core.FormatBluetooth(data["Address"]);
                        }
...
var AdministratorDevices = new AdministratorDevices();